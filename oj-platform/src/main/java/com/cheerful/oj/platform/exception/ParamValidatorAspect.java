 ...
 * @DESCRIPTION: AOP对请求的参数进行拦截
 *     {@link com.cheerful.oj.common.exception.ParamException}
 *     {@link AssertException}
 *  {@link ServerExceptionResolver}
 */
        ...
        for (Object arg : point.getArgs()) {
            ...
            for (Field field : fields) {
                ...
                if (field.isAnnotationPresent(Verify.class)) {
                    ...
                    //字符串的 非空校验
                    if (notNull) {
                        AssertException.assertMethod(!ObjectUtils.isEmpty(fieldObj), String.format("【%s】不能为空", name));
                    }
                    //是否有最大长度限制 断言判断
                    if (Integer.MAX_VALUE != maxLength) {
                        AssertException.assertMethod(maxLength > String.valueOf(fieldObj).length(), String.format("【%s】长度不合理，最大长度为【%s】", name, maxLength));
                    }
                    //是否有最小长度限制 断言判断
                    if (Integer.MIN_VALUE != minLength) {
                        AssertException.assertMethod(minLength < String.valueOf(fieldObj).length(), String.format("【%s】长度不合理，最小长度为【%s】", name, minLength));
                    }
                    //是否有正则校验
                    if (!"".equals(regular)) {
                        Pattern pattern = Pattern.compile(regular);
                        //断言判断正则
                        AssertException.assertMethod(pattern.matcher(String.valueOf(fieldObj)).matches(), String.format("参数【%s】不符合规则", name));
                    }
                }
            }
        }
